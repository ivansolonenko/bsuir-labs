...
namespace lab07
{
	static class Program
	{
		...
		static double Func(double[] x)
		{
			...
			return 100 * ((x[2] - 10 * theta) * (x[2] - 10 * theta) + (r - 1) * (r - 1)) + x[2] * x[2] * x[2];
			//return 100 * (x[0] - 1) * (x[0] - 1) + (x[1] - 2) * (x[1] - 2) + (x[2] - 3) * (x[2] - 3);
		}
		...
		// ReSharper restore UnusedParameter.Local
		{
			...
			D = new double[N];
			Console.Write("Начальный вектор: (");
			for (var i = 0; i < N; ++i)
			{
				D[i] = D_SOURCE;
				Console.Write(D[i] + " ");
